...
public class FrownMegaMenuRenderer extends MegaMenuRenderer {
	...
	protected void encodeRootSubmenuItem(FacesContext context, SubmenuItem submenuItem) throws IOException {
		...
		if(submenuItem.shouldRenderChildren()) {
            ...
            //GET
			if(submenuItem.getUrl() != null || submenuItem.getOutcome() != null) {
				...
				if(submenuItem.getTarget() != null) {
                    writer.writeAttribute("target", submenuItem.getTarget(), null);
                }
			}
            //POST
            else {
                ...
                UIComponent form = ComponentUtils.findParentForm(context, submenuItem);
                if(form == null) {
                    throw new FacesException("MenuItem must be inside a form element");
                }

                String command = submenuItem.isAjax() ? buildAjaxRequest(context, submenuItem, form) : buildNonAjaxRequest(context, submenuItem, form, clientId, true);
